...
ensure_dirs() {
  mkdir -p "$APP_DIR" "$TUNNELS_DIR"
  chmod 700 "$APP_DIR" || true
  chmod 700 "$TUNNELS_DIR" || true
}
...
write_conf() {
  ...
  ensure_dirs
  cat >"$f" <<EOF
# Generated by simple-gre (multi tunnel)
ROLE="${ROLE}"
PAIR_CODE="${PAIR_CODE}"
TUN_NAME="${TUN_NAME}"
...
ExecStop=/usr/local/sbin/simple-gre-down %i
...
tun="${1:-}"
[[ -n "${tun}" ]] || { echo "Usage: simple-gre-up <tunnel_name>" >&2; exit 2; }
...
tun="${1:-}"
[[ -n "${tun}" ]] || { echo "Usage: simple-gre-down <tunnel_name>" >&2; exit 2; }
...
migrate_legacy_if_needed() {
  ...
  if [[ -f "$LEGACY_CONF_FILE" ]]; then
    warn "Legacy config detected: $LEGACY_CONF_FILE"
    warn "Migrating to multi-tunnel format..."
...
show_info_one() {
  ...
  if ! read_conf "$tun"; then
    err "No configuration found for tunnel: $tun"
    return
  fi

  echo -e "${MAG}===== GRE Configuration: ${tun} =====${NC}"
  echo "Role:                ${ROLE:-N/A}"
...
do_edit() {
  ...
  if ! read_conf "$old_tun"; then
    err "No existing configuration found for: $old_tun"
    return
  fi
...
do_status_one() {
  ...
  if ! read_conf "$tun"; then
    err "No existing configuration found for: $tun"
    return
  fi

  echo -e "${MAG}===== GRE Status: ${tun} =====${NC}"
  ...
  if ping -c 3 -W 2 "$TUN_REMOTE_IP" >/dev/null 2>&1; then
    ...
    warn "Ping failed."
    warn "If GRE packets arrive but ping fails, check rp_filter=0 and firewall/proto 47."
  fi
}

do_status_all() {
  echo -e "${MAG}===== GRE Status: ALL TUNNELS =====${NC}"
  local t found="no"
  while IFS= read -r t; do
    [[ -z "$t" ]] && continue
    found="yes"
    ...
    if read_conf "$t"; then
      ...
      if ip link show "$t" >/dev/null 2>&1; then
        ...
        warn "Interface $t not present."
      fi
    else
      warn "Cannot read config for $t"
    fi
    echo
  done < <(list_tunnels)
...
do_delete() {
  ...
  if ! read_conf "$tun"; then
    err "No config found for tunnel: $tun"
    return
  fi
...
banner() {
  echo -e "${MAG}========================================${NC}"
  echo -e "${WHT}  ${APP_NAME}${NC}  ${CYA}(GRE Tunnel Manager - Multi)${NC}"
